...
// Task::Task() ensures every Task starts in a known, safe default
// state instead of having uninitialized int/bool fields.
Task::Task()
    : taskId(0), priority(1), isCompleted(false), daysToComplete(0),
      dueDay(1), dueMonth(1), dueYear(2026) {}

// Task::markDone() centralizes what "completing a task" means -- this
// exact 4-line sequence used to be copy-pasted in six different places
// across task.cpp and dashboard.cpp. Now there is one place to look.
int Task::markDone() {
    isCompleted = true;
    dateCompleted = todayDateString();
    int gained = xpForPriority(priority);
    totalXP += gained;
    return gained;
}
...
void markCompleted() {
    ...
    for (auto& t : allTasks) {
        if (t.taskId == id) {
            ...
            // Task owns the logic for what "completing itself" means.
            int gained = t.markDone();
            saveUserData(); // Save automatically!
