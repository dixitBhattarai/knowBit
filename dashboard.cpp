...
#include <algorithm>
#include <sstream>
#include <memory>      // for unique_ptr -- automatic cleanup, no manual delete
#include <functional>  // for std::function -- lets SpecialAchievement take a custom condition
...
struct ModalState {
    ...
    char tfCat[32]     = "";
    char tfPri[4]      = "";
    // Bug fix: holds the typed-in text when the "Other" category chip is
    // selected, so a custom category can actually be entered (previously
    // selecting "Other" just stored the literal word "Other").
    char tfCatOther[32] = "";
    int  activeField   = 0;
    string errorMsg    = "";
...
// ── Bug fix: Priority selection menu (replaces manual "1-5" text entry) ──
// Still stores the value in m.tfPri exactly like before (a short numeric
// string parsed with atoi() at submit time) -- only the INPUT method changed.
{
    Rectangle priRow = {fx, fy + 2*76.0f, fw, (float)fh};
    DrawText("Priority", (int)priRow.x, (int)priRow.y - 20, 13, C_TEXT_DIM);
    static const char* PRI_LABELS[] = { "Low", "Medium", "High" };
    static const int   PRI_VALUES[] = { 1, 3, 5 };
    float px = priRow.x;
    for (int i = 0; i < 3; i++) {
        int tw = MeasureText(PRI_LABELS[i], 14);
        float chipW = tw + 28.0f;
        Rectangle chip = { px, priRow.y, chipW, priRow.height };
        bool chipHov = CheckCollisionPointRec(mouse, chip);
        char curVal[4]; snprintf(curVal, sizeof(curVal), "%d", PRI_VALUES[i]);
        bool chipSel = (strcmp(m.tfPri, curVal) == 0);
        DrawRoundRect(chip, 0.3f, chipSel ? C_ORANGE : (chipHov ? C_CARD_HOV : C_DARKGRAY));
        DrawRoundRectLines(chip, 0.3f, 1.0f, chipSel ? C_ORANGE : C_BORDER);
        DrawText(PRI_LABELS[i], (int)chip.x + 14, (int)chip.y + 9, 14, chipSel ? C_BG : C_WHITE);
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && chipHov) {
            snprintf(m.tfPri, sizeof(m.tfPri), "%d", PRI_VALUES[i]);
        }
        px += chipW + 8;
    }
}
...
// Bug fix: when "Other" is selected, show a text box for the actual
// custom category instead of just silently storing the word "Other".
bool customCatMode = (strcmp(m.tfCat, "Other") == 0);
Rectangle otherBox = {fx, caty + 36, fw, 32};
if (customCatMode) {
    DrawInputBox("Enter your custom category", m.tfCatOther, otherBox, true, C_ORANGE);
}
    ...
    if (IsKeyPressed(KEY_TAB)) {
        // Only 2 real typed fields now (Name, Description) since Priority
        // became buttons above -- cycling range shrank from 3 to 2.
        m.activeField = (m.activeField + 1) % 2;
    }
    // Bug fix: while typing a custom category, keyboard input goes only to
    // that box (not Name/Description) -- otherwise both fields would try
    // to consume the same GetCharPressed() queue in the same frame.
    if (customCatMode) {
        HandleTextInput(m.tfCatOther, sizeof(m.tfCatOther),
                        (IsKeyPressed(KEY_BACKSPACE) || IsKeyPressedRepeat(KEY_BACKSPACE)) ? KEY_BACKSPACE : 0);
    } else {
        HandleTextInput(fields[m.activeField].buf,
                        fields[m.activeField].maxLen,
                        // Bug fix: single-press backspace (was hold-only:
                        // IsKeyPressedRepeat only fires on the OS repeat event)
                        (IsKeyPressed(KEY_BACKSPACE) || IsKeyPressedRepeat(KEY_BACKSPACE)) ? KEY_BACKSPACE : 0);
    }
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || enterConfirm || escCancel) {
        ...
        if (addHov) {
            ...
            if (TextLength(m.tfName) == 0) {
            ...
            } else if (customCatMode && TextLength(m.tfCatOther) == 0) {
                m.errorMsg = "Please enter your custom category.";
            } else {
                Task t;
                t.taskId          = nextAvailableTaskId();  // auto-assigned — never asked from the user
                t.taskName        = m.tfName;
                t.taskDescription = m.tfDesc;
                // Bug fix: use the typed custom category when "Other" was
                // selected, instead of literally storing the word "Other".
                t.taskCategory    = customCatMode ? string(m.tfCatOther) : string(m.tfCat);
                ...
                memset(m.tfCatOther,0,sizeof(m.tfCatOther));
...
static void DrawEditModal(ModalState& m, Vector2 mouse) {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        if (CheckCollisionPointRec(mouse, r1)) m.activeField = 0;
        if (CheckCollisionPointRec(mouse, r2)) m.activeField = 1;
    }

    // Bug fix: Priority selection menu (replaces manual "1-5" text entry).
    // Still stores into m.efPri exactly like before -- empty means "leave
    // the task's current priority unchanged" (same as the original code).
    DrawText("Priority", (int)r0.x, (int)r0.y - 20, 13, C_TEXT_DIM);
    {
        static const char* PRI_LABELS[] = { "Low", "Medium", "High" };
        static const int   PRI_VALUES[] = { 1, 3, 5 };
        float px = r0.x;
        for (int i = 0; i < 3; i++) {
            int tw = MeasureText(PRI_LABELS[i], 14);
            float chipW = tw + 28.0f;
            Rectangle chip = { px, r0.y, chipW, r0.height };
            bool chipHov = CheckCollisionPointRec(mouse, chip);
            char curVal[4]; snprintf(curVal, sizeof(curVal), "%d", PRI_VALUES[i]);
            // Highlight either what was just picked this session, or (if
            // nothing picked yet) the task's current priority value.
            bool chipSel = (TextLength(m.efPri) > 0) ? (strcmp(m.efPri, curVal) == 0)
                                                       : (t.priority == PRI_VALUES[i]);
            DrawRoundRect(chip, 0.3f, chipSel ? C_TEAL : (chipHov ? C_CARD_HOV : C_DARKGRAY));
            DrawRoundRectLines(chip, 0.3f, 1.0f, chipSel ? C_TEAL : C_BORDER);
            DrawText(PRI_LABELS[i], (int)chip.x + 14, (int)chip.y + 9, 14, chipSel ? C_BG : C_WHITE);
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && chipHov) {
                snprintf(m.efPri, sizeof(m.efPri), "%d", PRI_VALUES[i]);
            }
            px += chipW + 8;
        }
    }

    DrawInputBox("Category / Type",  m.efCat,  r1, m.activeField==0, C_TEAL);
    DrawInputBox("Days to Due Date", m.efDays, r2, m.activeField==1, C_TEAL);

    char* bufs[2] = {m.efCat, m.efDays};
    int   maxL[2] = {32, 8};
    HandleTextInput(bufs[m.activeField], maxL[m.activeField],
                    (IsKeyPressed(KEY_BACKSPACE) || IsKeyPressedRepeat(KEY_BACKSPACE)) ? KEY_BACKSPACE : 0);
    if (IsKeyPressed(KEY_TAB)) {
        // Only 2 real typed fields now (Category, Days) since Priority
        // became buttons above -- cycling range shrank from 3 to 2.
        m.activeField = (m.activeField + 1) % 2;
    }
...
static void DrawFocusPanel(int taskIdx, FocusTimer& ft, Vector2 mouse) {
    ...
    if (ft.inputMode) {
        ...
        HandleTextInput(ft.setMin, 6, (IsKeyPressed(KEY_BACKSPACE)||IsKeyPressedRepeat(KEY_BACKSPACE))?KEY_BACKSPACE:0);
...
// ─────────────────────────────────────────────────────────────
// OOP Concept: Abstract Base Class + Pure Virtual Function
// "= 0" makes isEarned() pure virtual, which makes Achievement abstract --
// you can never create a plain Achievement object, only one of the
// concrete subclasses below.
class Achievement {
public:
    string icon, name;
    Achievement(string ic, string nm) : icon(ic), name(nm) {}
    virtual bool isEarned() const = 0;   // pure virtual -> abstract class
    virtual ~Achievement() {}            // virtual destructor (correct practice for base classes)
};

// OOP Concept: Inheritance
// Earned by hitting a count of tasks (total created, or total completed).
class TaskCountAchievement : public Achievement {
    int threshold; bool completedOnly;
public:
    TaskCountAchievement(string ic, string nm, int t, bool onlyCompleted)
        : Achievement(ic, nm), threshold(t), completedOnly(onlyCompleted) {}
    // OOP Concept: Runtime Polymorphism / Virtual Function Override
    // This isEarned() runs instead of the base class's whenever it's
    // called through a base-class pointer pointing at a TaskCountAchievement.
    bool isEarned() const override {
        int count = 0;
        for (auto& t : allTasks) if (!completedOnly || t.isCompleted) count++;
        return count >= threshold;
    }
};

// OOP Concept: Inheritance
// Earned by hitting a streak length (current streak, or best-ever streak).
class StreakAchievement : public Achievement {
    int threshold; bool useMaxStreak;
public:
    StreakAchievement(string ic, string nm, int t, bool useMax)
        : Achievement(ic, nm), threshold(t), useMaxStreak(useMax) {}
    bool isEarned() const override {
        return (useMaxStreak ? activeUserMaxStreak : activeUserStreak) >= threshold;
    }
};

// OOP Concept: Inheritance
// Earned by hitting a total XP threshold.
class XPAchievement : public Achievement {
    int threshold;
public:
    XPAchievement(string ic, string nm, int t) : Achievement(ic, nm), threshold(t) {}
    bool isEarned() const override { return totalXP >= threshold; }
};

// OOP Concept: Inheritance
// For one-off conditions that don't fit the categories above -- takes any
// custom condition as a lambda/std::function, so we don't need a brand
// new subclass for every single special rule.
class SpecialAchievement : public Achievement {
public:
    function<bool()> condition;
    SpecialAchievement(string ic, string nm, function<bool()> cond)
        : Achievement(ic, nm), condition(cond) {}
    bool isEarned() const override { return condition(); }
};

// OOP Concept: Smart Pointers (std::unique_ptr)
// Returns unique_ptr<Achievement> -- each Achievement is automatically
// freed when the vector goes out of scope at the end of DrawProfileView()
// every frame. No manual delete, no memory leak.
static vector<unique_ptr<Achievement>> ComputeAchievements() {
    vector<unique_ptr<Achievement>> a;
    // OOP Concept: Base-class pointer holding derived-class objects
    // Every element here is stored as unique_ptr<Achievement> (the BASE
    // class) even though the actual object underneath is one of the four
    // derived types above. Calling ->isEarned() later resolves at runtime
    // to whichever derived class each pointer actually points to.
    a.push_back(make_unique<TaskCountAchievement>("1",   "First Task",     1,   false));
    a.push_back(make_unique<TaskCountAchievement>(">10", "Task Slayer",    10,  true));
    a.push_back(make_unique<XPAchievement>       ("100", "Century",        100));
    a.push_back(make_unique<StreakAchievement>   ("3d",  "Streak Starter", 3,   false));
    a.push_back(make_unique<StreakAchievement>   ("7d",  "Streak Master",  7,   true));
    a.push_back(make_unique<SpecialAchievement>  ("P5",  "High Roller", []{
        for (auto& t : allTasks) if (t.isCompleted && t.priority >= 5) return true;
        return false;
    }));
    a.push_back(make_unique<SpecialAchievement>  ("ALL", "Perfectionist", []{
        int total = (int)allTasks.size(), completed = 0;
        for (auto& t : allTasks) if (t.isCompleted) completed++;
        return total > 0 && completed == total;
    }));
    a.push_back(make_unique<XPAchievement>       ("Lv2", "Rising Star", 100)); // level>=2 == xp>=100
    return a;
}
...
static void DrawProfileView(Vector2 mouse) {
    ...
    vector<unique_ptr<Achievement>> ach = ComputeAchievements();
    int cols = 4;
    ...
    for (size_t i = 0; i < ach.size(); i++) {
        ...
        // OOP Concept: Runtime Polymorphism via base-class pointer
        // ach[i] is an Achievement* (base class) but isEarned() runs the
        // actual derived class's implementation depending on what it is.
        bool earned = ach[i]->isEarned();
        Color hexCol = earned ? C_ORANGE : C_BORDER;
        Color txtCol = earned ? C_WHITE  : C_TEXT_DIM;
        DrawPoly({ bx, by }, 6, 34, 90, earned ? Color{255,128,0,40} : Color{40,55,75,40});
        DrawPolyLines({ bx, by }, 6, 34, 90, hexCol);
        int iw = MeasureText(ach[i]->icon.c_str(), 14);
        DrawText(ach[i]->icon.c_str(), (int)(bx - iw / 2.0f), (int)(by - 7), 14, hexCol);
        int nwid = MeasureText(ach[i]->name.c_str(), 12);
        DrawText(ach[i]->name.c_str(), (int)(bx - nwid / 2.0f), (int)(by + 42), 12, txtCol);
    }
}
